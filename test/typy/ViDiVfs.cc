...
int ViDiVfs::Visit(visitproc visit, void* arg) {
...
void ViDiVfs::CopyFrom(const ViDiVfs& from) {
	if (&from == this) { return; }
...
void ViDiVfs::MergeFrom(const ViDiVfs& from) {
...
void ViDiVfs::SerializeWithCachedSizes(CodedOutputStream* output) const {
	switch (_tag) {
...
bool ViDiVfs::MergePartialFromCodedStream(CodedInputStream* input) {
	BEGINE_READ_CASE(2)
...
bool ViDiVfs::fromPyObject(PyObject* value) {
...
PyObject* ViDiVfs::Json(bool slim) {
	if (!slim || _tag != 0) {
		switch (_tag) {
...
ViDiVfs* ViDiVfs::FromJson(PyObject* json) {
	ViDiVfs* object = new ViDiVfs;
...
bool CheckAndSet(PyObject* arg, ViDiVfs*& value, const char* err) {
	if (arg == Py_None) {
