...
int ViDif::Visit(visitproc visit, void* arg) {
...
void ViDif::CopyFrom(const ViDif& from) {
	if (&from == this) { return; }
...
void ViDif::MergeFrom(const ViDif& from) {
...
void ViDif::SerializeWithCachedSizes(CodedOutputStream* output) const {
	switch (_tag) {
...
bool ViDif::MergePartialFromCodedStream(CodedInputStream* input) {
	BEGINE_READ_CASE(2)
...
bool ViDif::fromPyObject(PyObject* value) {
...
ViDif* ViDif::FromJson(PyObject* json) {
	ViDif* object = new ViDif;
...
bool CheckAndSet(PyObject* arg, ViDif*& value, const char* err) {
	if (arg == Py_None) {
